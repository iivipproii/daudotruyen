...
export function StoryDetailHero({ story, chapters, continueProgress, firstChapter, onFollow, onFavorite, onReport, onCombo, pendingAction }) {
  ...
  const chapterPrice = Number(story.chapterPrice || story.price || 1);
  const comboPrice = Math.max(49, chapterPrice * Math.max(chapterCount, 1));
  return (
    <section className="sd-hero" style={{ '--sd-bg': `url("${story.banner || story.cover || coverFallback}")` }}>
      ...
      <div className="sd-hero-copy">
        ...
        <div className="sd-offer-strip">
          ...
          <span><b>{chapterPrice}</b> Đậu/chương VIP</span>
          ...
          <div key={chapter.id} className={chapter.isPremium ? 'vip' : ''}>
            <Link to={`/truyen/${story.slug}/chuong/${chapter.number}`}>
              ...
              <small>
                ...
                <b className={`sd-chapter-badge ${chapter.isPremium ? 'premium' : 'free'}`}>{chapter.isPremium ? `${chapter.price || chapterPrice} Đậu` : 'Miễn phí'}</b>
...
export function RelatedStoriesSidebar({ story, chapters = [], related, authorStories, onCombo }) {
  ...
  const chapterPrice = Number(story.chapterPrice || story.price || 1);
  const comboPrice = Math.max(49, chapterPrice * Math.max(chapters.length, 1));
  return (
...
export function PurchaseChapterModal({ story, chapters, target, onClose, onConfirm }) {
  ...
  const price = isCombo ? comboPrice : target.chapter?.price || chapterPrice;
